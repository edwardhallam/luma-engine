...
class RequirementAnalysisRequest(BaseModel):
    ...
    user_request: str = Field(..., min_length=10, description="Natural language requirement description")
...
class RequirementAnalysisResponse(BaseModel):
    ...
    specification: Optional[Dict[str, Any]] = Field(default=None, description="Generated deployment specification")
    confidence_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Confidence in the analysis")
    ...
    alternatives: List[Dict[str, Any]] = Field(default_factory=list, description="Alternative specifications")
...
class RequirementRefinementRequest(BaseModel):
    ...
    modifications: Optional[Dict[str, Any]] = Field(default=None, description="Specific modifications requested")
    validation_errors: Optional[List[str]] = Field(default=None, description="Validation errors to address")
...
class RequirementTemplate(BaseModel):
    ...
    examples: List[str] = Field(default_factory=list, description="Example requirements")
...
class RequirementHistory(BaseModel):
    ...
    final_specification: Dict[str, Any] = Field(..., description="Final deployment specification")
    
    # Analysis process
    iterations: List[Dict[str, Any]] = Field(default_factory=list, description="Analysis iterations")
    feedback_provided: List[str] = Field(default_factory=list, description="User feedback given")
    
    # Outcome
    deployed: bool = Field(default=False, description="Whether specification was deployed")
    deployment_id: Optional[str] = Field(default=None, description="Resulting deployment ID")
    success_rating: Optional[int] = Field(default=None, ge=1, le=5, description="User success rating")
    ...
    total_duration_seconds: Optional[int] = Field(default=None, description="Total analysis duration")
...
class RequirementInsight(BaseModel):
    ...
    # Pattern details
    pattern_keywords: List[str] = Field(default_factory=list, description="Keywords associated with pattern")
    common_specifications: Dict[str, Any] = Field(default_factory=dict, description="Common specification patterns")
    
    # Recommendations
    optimization_suggestions: List[str] = Field(default_factory=list, description="Optimization suggestions")
    pitfalls_to_avoid: List[str] = Field(default_factory=list, description="Common pitfalls")
    
    # Metadata
    first_observed: datetime = Field(..., description="First time this pattern was observed")
