...
class ImmediateAction(BaseModel):
    ...
    estimated_time: Optional[int] = Field(
        default=None, description="Estimated time in seconds"
    )
...
class ErrorDiagnosisRequest(BaseModel):
    ...
    previous_fixes: Optional[List[str]] = Field(
        default=None, description="Previously attempted fixes"
    )

    # Context information
    deployment_id: Optional[str] = Field(
        default=None, description="Related deployment ID"
    )
    ...
    # Timing information
    error_started: Optional[datetime] = Field(
        default=None, description="When error first occurred"
    )
    last_working: Optional[datetime] = Field(
        default=None, description="Last known working state"
    )
...
class ErrorDiagnosisResponse(BaseModel):
    ...
    error_analysis: Optional[ErrorAnalysis] = Field(
        default=None, description="Error analysis"
    )
    immediate_actions: List[ImmediateAction] = Field(
        default_factory=list, description="Immediate actions"
    )
    root_cause_fixes: List[RootCauseFix] = Field(
        default_factory=list, description="Root cause fixes"
    )
    ...
    confidence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence in diagnosis"
    )
    estimated_resolution_time: Optional[str] = Field(
        default=None, description="Estimated resolution time"
    )
    requires_manual_intervention: bool = Field(
        default=False, description="Requires manual intervention"
    )
    ...
    error_message: Optional[str] = Field(
        default=None, description="Error message if diagnosis failed"
    )
...
class ErrorResolutionRequest(BaseModel):
    ...
    selected_actions: List[int] = Field(
        ..., description="Indices of selected actions to execute"
    )
    force_execution: bool = Field(
        default=False, description="Force execution even if risky"
    )
...
class ErrorResolutionResponse(BaseModel):
    ...
    failed_actions: List[Dict[str, Any]] = Field(
        default_factory=list, description="Actions that failed"
    )

    # Status
    resolution_status: str = Field(..., description="Resolution status")
    error_resolved: bool = Field(
        default=False, description="Whether error was resolved"
    )

    # Metadata
    started_at: datetime = Field(..., description="Resolution start time")
    completed_at: Optional[datetime] = Field(
        default=None, description="Resolution completion time"
    )
...
class ErrorPattern(BaseModel):
    ...
    success_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Resolution success rate"
    )
    confidence_level: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Pattern confidence"
    )
    ...
    child_patterns: List[str] = Field(
        default_factory=list, description="Child pattern IDs"
    )
...
class ErrorInsight(BaseModel):
    ...
    preventive_actions: List[str] = Field(
        default_factory=list, description="Preventive actions"
    )
