...
class LLMProviderStatus(BaseModel):
    ...
    primary: bool = Field(default=False, description="Whether this is the primary provider")
    fallback: bool = Field(default=False, description="Whether this is a fallback provider")
    ...
    success_rate: float = Field(default=0.0, ge=0.0, le=1.0, description="Success rate over last 24h")
...
class LLMRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(default=None, ge=0.0, le=2.0, description="Generation temperature")
...
class LLMUsageStats(BaseModel):
    ...
    median_response_time: float = Field(..., description="Median response time in milliseconds")
    ...
    cost_per_request: Optional[float] = Field(default=None, description="Average cost per request")
    
    # Operation breakdown
    operations: Dict[str, int] = Field(default_factory=dict, description="Requests by operation type")
    models_used: Dict[str, int] = Field(default_factory=dict, description="Requests by model")
...
class LLMConfiguration(BaseModel):
    ...
    request_sanitization: bool = Field(default=True, description="Enable request sanitization")
...
class LLMHealthCheck(BaseModel):
    ...
    # Provider health
    providers: Dict[str, Dict[str, Any]] = Field(..., description="Provider health status")
    primary_provider_healthy: bool = Field(..., description="Whether primary provider is healthy")
    fallback_available: bool = Field(..., description="Whether fallback providers are available")
...
class LLMPromptTemplate(BaseModel):
    ...
    optional_variables: List[str] = Field(default_factory=list, description="Optional input variables")
    
    # Configuration
    default_parameters: Dict[str, Any] = Field(default_factory=dict, description="Default parameters")
    recommended_models: List[str] = Field(default_factory=list, description="Recommended models")
