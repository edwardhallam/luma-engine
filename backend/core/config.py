...
class Settings(BaseSettings):
    ...
    host: str = Field(
        "0.0.0.0", env="HOST"
    )  # nosec B104 - Intentional for container deployment
    ...
    llm_fallback_providers: str = Field(
        "anthropic,ollama", env="LLM_FALLBACK_PROVIDERS"
    )
    ...
    gitlab_project_namespace: str = Field(
        "aid-deployments", env="GITLAB_PROJECT_NAMESPACE"
    )
