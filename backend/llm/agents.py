...
class RequirementAgent:
    ...
    def _setup_agent(self) -> None:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert infrastructure architect specialized in analyzing deployment requirements.
    ...
    def _validate_service_type(self, service_type: str) -> str:
        ...
        if service_type.lower() in supported_types:
            ...
            closest = min(
                supported_types, key=lambda x: abs(len(x) - len(service_type))
            )
    ...
    def _estimate_resources(self, service_description: str) -> str:
        ...
        if any(
            ...
            for word in ["database", "db", "postgresql", "mysql"]
        ):
    ...
    def _check_compatibility(self, requirements: str) -> str:
        ...
        return json.dumps(
            {"compatible": len(issues) == 0, "issues": issues, "warnings": warnings}
        )
    ...
    def _calculate_confidence(
        self, specification: Dict[str, Any], user_request: str
    ) -> float:
    ...
    def _extract_assumptions(self, response: str) -> List[str]:
        ...
        for indicator in assumption_indicators:
            if indicator in response.lower():
                assumptions.append(
                    f"Made assumptions based on '{indicator}' in analysis"
                )
...
class ErrorAnalysisAgent:
    ...
    def _setup_agent(self) -> None:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert DevOps troubleshooter specialized in infrastructure deployments.
    ...
    async def diagnose_error(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "diagnosis": None}
    ...
    def _check_known_issues(self, error_pattern: str) -> str:
        ...
        return json.dumps(
            {"known_issue": False, "similar_issues": [], "documented_solutions": []}
        )
