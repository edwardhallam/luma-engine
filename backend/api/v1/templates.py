...
async def get_template(
    ...
    try:
        template = await service.get_template(
            template_id, include_preview=include_preview
        )
        if not template:
            raise HTTPException(
...
async def create_template(
    request: TemplateRequest, service: TemplateService = Depends(get_template_service)
) -> TemplateResponse:
...
async def get_template_usage(
    ...
    try:
        ...
        logger.error(
            f"Unexpected error getting usage stats for template {template_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
...
async def get_recommended_templates(
    ...
    user_context: Optional[str] = Query(
    ...
    try:
        recommendations = await service.get_recommended_templates(
            limit=limit, user_context=user_context
        )
