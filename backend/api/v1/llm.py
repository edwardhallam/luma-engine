    ...
    try:
        ...
        if result["success"]:
            ...
            return ErrorDiagnosisResponse(
                ...
                monitoring_recommendations=diagnosis.get(
                ...
                requires_manual_intervention=diagnosis.get(
...
async def health_check(
    ...
    try:
        ...
        available_providers = [
            name for name, info in status_info.items() if info["available"]
        ]
        primary_healthy = any(
            info.get("primary") and info["available"] for info in status_info.values()
        )
...
async def llm_request(
    request: LLMRequest, service: LLMService = Depends(get_llm_service)
) -> LLMResponse:
    """Generic LLM request."""
    try:
