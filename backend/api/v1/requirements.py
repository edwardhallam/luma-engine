...
async def get_requirement_template(
    ...
    try:
        ...
        logger.error(f"Unexpected error getting requirement template {template_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error"
        )
...
async def provide_feedback(
    ...
    try:
        ...
        logger.error(f"Unexpected error saving feedback for analysis {analysis_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error"
        )
...
async def get_requirement_suggestions(
    ...
    try:
        suggestions = await service.get_requirement_suggestions(partial_request, context)
